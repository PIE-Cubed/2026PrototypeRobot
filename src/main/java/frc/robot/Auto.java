...
public class Auto {
    ...
    public Auto(Drive drive/*, Shooter shooter, Hopper hopper, Climber climber*/) {
        this.drive = drive;
    ...
    public int centerOutClimbOut() {
        ...
        int pathStatus = choreoPathFollower();
    ...
    public void initializeTrajectory(String trajectoryFileName) {
        currentTrajectory = Choreo.loadTrajectory(trajectoryFileName);
        drive.resetPose(currentTrajectory.get().getInitialPose(AllianceUtil.isRedAlliance()).orElse(Pose2d.kZero));
    }

    /**
     * Requires {@code initializeTrajectory("name")} to be run before this or before running a new path.
     */
    public int choreoPathFollower() {
